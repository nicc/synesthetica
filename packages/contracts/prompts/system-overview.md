...
**Beat grid anchor.** The beat grid is anchored at session-time zero — beat `N` sits at time `N × beatMs` where `beatMs = 60000 / tempo`. Not at the moment `set_tempo` was called. The visual grid is present in the rhythm view whether the metronome is on or off. If the user changes tempo mid-session, events from before the change use the old `beatMs` for correlation; this primer doesn't ship a tempo-change history today, so treat pre-change correlation as approximate.
