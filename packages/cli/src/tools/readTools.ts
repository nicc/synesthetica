...
export const getStartedTool: ToolSpec = {
  ...
  description:
    "Return the full Synesthetica primer: pipeline narrative + every macro (with range, default, directionality), session controls, system concepts, lenses, tools with aliases/notes/examples, resources, session-time semantics, and preset workflow. Call this once per conversation before acting on other Synesthetica tools — everything the LLM needs to interpret the user's musical requests is in this response.",
  inputSchema: {
    type: "object",
    properties: {
